...
func SelectMode(screen *termy.Termy, _ internal.UserOptions) (internal.Mode, internal.Action) {
	screen.ClearScreen()
	screen.UseDefault()
	screen.HideCur()
	screen.Send()
	...
	typewriter.WriteCentered(title, cols)

	menuSep := 3
	menuHeight := menuSep*(len(menu)-1) + 1
	...
	for {
		...
		screen.SetFg(6)
		screen.Send()
		screen.MoveTo(1, rows - marginTop)
		screen.ClearToEOL()
		typewriter.WriteCentered(menu[selected].Description, cols)

		res, _ := input.GetChar()
		switch res {
		case 'q':
			return 0, internal.Quit
		case '\x1b':
		...
		case '\n':
			return menu[selected].Mode, internal.NoOp
		}
	}
}
